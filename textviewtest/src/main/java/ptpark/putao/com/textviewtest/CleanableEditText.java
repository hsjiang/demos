...
public class CleanableEditText extends android.support.v7.widget.AppCompatEditText implements View.OnFocusChangeListener, TextWatcher {
    ...
    public void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        Log.d("TVTestSpaceFilter", "onTextChanged: " + text.toString() + " ," + start + " ," + lengthBefore + " ," + lengthAfter);
...
//        }
    ...
    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        ...
        Log.d("TVTestSpaceFilter", "beforeTextChanged: " + beforeStr + " ," + start + " ," + count + " ," + after);
    }
    ...
    public void afterTextChanged(Editable s) {
        boolean isVisible = getText().length() >= 1;
        setClearDrawableVisible(isVisible);
        Log.d("TVTestSpaceFilter", "afterTextChanged s: " + s.toString());
    }
    ...
    public static class SpaceInputFilter implements InputFilter {
        @Override
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
            Log.d("TVTestSpaceFilter", "source: " + source + " ,start: " + start + " ,end: " + end);
            Log.d("TVTestSpaceFilter", "dest: " + dest + " ,dstart: " + dstart + " ,dend: " + dend);
            String trimSource = source.toString().trim();
            String filterResult = TextUtils.isEmpty(trimSource) ? "" : trimSource;
            Log.d("TVTestSpaceFilter", "filterResult: " + filterResult);
            return TextUtils.isEmpty(trimSource) ? "" : null;
        }
    }

    public static class NewSpaceInputFilter implements InputFilter {
        @Override
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
