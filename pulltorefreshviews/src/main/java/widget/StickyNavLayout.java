...
import com.example.riven_chris.pulltorefreshviews.R;
...
public class StickyNavLayout extends LinearLayout {
    ...
    public interface ScrollObserver {
        void onToggleTopMenu(boolean show);
    }
    ...
    private boolean isTopMenuShown;
    private ScrollObserver scrollObserver = null;
    ...
    public StickyNavLayout(Context context, AttributeSet attrs) {
        ...
        mTopMenuHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 45,
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mTopViewHeight = mTop.getMeasuredHeight() - mTopMenuHeight;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dy) > mTouchSlop) {
                    ...
                    if (mInnerScrollView instanceof ScrollView) {
                        ...
                        PullToRefreshListView rv = (PullToRefreshListView) mInnerScrollView;
                        if (!isTopHidden || (!android.support.v4.view.ViewCompat.canScrollVertically(rv, -1) && isTopHidden && dy > 0)) {
                            initVelocityTrackerIfNotExists();
                            mVelocityTracker.addMovement(ev);
                            mLastY = y;
                            if (rv.getRefreshableView() instanceof ListView) {
                                ListView lv = rv.getRefreshableView();
                                View c = lv.getChildAt(lv.getFirstVisiblePosition());
                                // 如果topView没有隐藏
                                // 或sc的listView在顶部 && topView隐藏 && 下拉，则拦截

                                if (dy > 0 && mTop.getTop() >= 0 && !isTopHidden) {
                                    return false;
                                }
                                if (!isTopHidden || //
                                        (c != null //
        ...
        if (getScrollY() > getScrollX() && getScrollY() != mTopViewHeight) {
            return true;
        } else
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Log.i("TAG", "dy = " + dy + " , y = " + y + " , mLastY = " + mLastY);
    ...
    public void scrollTo(int x, int y) {
        ...
        if (mInnerScrollView instanceof PullToRefreshListView) {
            PullToRefreshListView ptr = (PullToRefreshListView) mInnerScrollView;
            if (isTopHidden) {
                ptr.setPullToRefreshEnabled(false);
            } else {
                ptr.setPullToRefreshEnabled(true);
            }
        }

        if (y == (mNav.getTop() - mTopMenuHeight)) {
            if (!isTopMenuShown) {
                if (scrollObserver != null)
                    scrollObserver.onToggleTopMenu(true);
                isTopMenuShown = true;
            }
        } else {
            if (isTopMenuShown) {
                if (scrollObserver != null)
                    scrollObserver.onToggleTopMenu(false);
